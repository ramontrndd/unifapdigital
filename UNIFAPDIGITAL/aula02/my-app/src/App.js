...
if (loading) return <div className='bg-gray-900 text-white'>Carregando...</div>
    ...
    <ul className='space-y-2 '>
      {post.map (post => (
        <li key={post.id} className="bg-gray-100 p-5 rounded-md">
          <h2 className="text-xl font-bold">{post.id}.{post.title} </h2>
...
const Home = () => {
  ...
  return (
    <div className={`flex  flex-col items-center justify-center h-screen ${isDarkMode ? 'bg-gray-900 text-white transition-all duration-500': 'bg-white text-gray-900 transition-all duration-500'}`}>
      ...
      <button onClick={toggleTheme} className=' mt-2 border-transparent border rounded-lg text-sm p-5 bg-purple-500 font-bold uppercase'>
